...
# Modified version: receive() no longer prints RSSI lines or the
# "receive message from node address..." line, and trims the trailing
# RSSI byte correctly based on self.rssi instead of always dropping it.
...
# "20,868,Hello World"
    ...
    # Persistent buffer across calls, in case a packet's bytes are split
    # across two separate ser.read() calls.
    _rx_buffer = b''
    ...
    def receive(self):
        ...
        if self.ser.inWaiting() > 0:
            time.sleep(0.5)
            self._rx_buffer += self.ser.read(self.ser.inWaiting())

        # Each received over-the-air packet arrives on the UART as:
        #   [3-byte header: sender addr hi, addr lo, freq offset]
        #   [payload bytes, always ending in \r\n since sender sends
        #    one complete HAN line per packet]
        #   [1 rssi byte, only present if self.rssi is True]
        #
        # When packets arrive faster than receive() is polled, more than
        # one complete packet can be sitting in the buffer at once - this
        # loop walks through and extracts as many complete packets as are
        # available, leaving any incomplete trailing bytes buffered for
        # the next call.
        while True:
            ...
            search_start = 3
            eol = self._rx_buffer.find(b'\r\n', search_start)
            if eol == -1:
                # payload not fully received yet, wait for more bytes
                break

            payload_end = eol + 2  # include the \r\n itself
            frame_end = payload_end + (1 if self.rssi else 0)

            if len(self._rx_buffer) < frame_end:
                # rssi byte not arrived yet, wait for more bytes
                break

            payload = self._rx_buffer[3:payload_end]
