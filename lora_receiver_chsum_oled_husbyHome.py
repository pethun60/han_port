...
#
#    Receive-side script for the husbyHome site. Reads raw bytes directly
#    from a Waveshare USB-to-LoRa dongle (/dev/ttyACM0, transparent
#    serial-over-radio bridge - no addressed packet protocol, unlike the
#    GPIO SX126x HAT used on rpizero3/rpizero4), reassembles/validates HAN
#    telegrams, drives an I2C SSD1306 OLED status display, and publishes
#    every parsed value to MQTT with retained Home Assistant discovery
#    configs. Telegram parsing/MQTT logic ported from
#    lora_receiver_chsum_oled_v2.py (rpizero4).
#
...
# OBIS code (rather than fixed line position) means every documented value
# gets picked up regardless of line order/count.
OBIS_KEY_MAP = {
    '0-0:1.0.0':  'datevalue',
...
# and the topic namespace, so husbyHome and Kronudden don't collide.
SITE_ID = 'husbyHome'
SITE_NAME = 'HusbyHome'
...
def main():
    ...
    with serial.Serial(args.port, args.baud, timeout=1) as ser, open(args.output, 'ab') as fout:
        buf = b''
        while True:
